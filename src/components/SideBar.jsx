...
import { useGetCheckingAccount } from "../hooks/useGetCheckingAccount";
...
function SideBar() {
  ...
  const {
    ...
    error: accountError,
  } = useGetCheckingAccount();

  // Determine if the sidebar is in a loading state
  const isLoading = isNameLoading || isAccountLoading;
      ...
      <div className="inner-homepage">
        ...
        <div className="bg-[#B7DC9D] p-4 rounded-xl w-full flex flex-col items-center h-full">
            ...
            {isLoading ? (
              ...
              <>
                ...
                <p className="font-semibold text-white text-4xl mb-1">
                  {nameError ? "Error loading name" : customerName}
                </p>
          ...
          <div className="mt-2 w-full pt-4">
            <ButtonSidebar
              icon={<ArrowLeftEndOnRectangleIcon className="w-8" />}
